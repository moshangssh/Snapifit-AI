...
export type ASRiskCategory =
  | "axial_loaded_lower" // 重轴向下肢负重：杠铃深蹲 / 硬拉
  | "overhead_press" // 负重过顶按压：杠铃 / 哑铃推举、借力推
  | "olympic_lift" // 奥举 / 爆发：抓举等
...
export const AS_RISK_CATEGORY_LABELS: Record<ASRiskCategory, string> = {
  axial_loaded_lower: "杠铃下肢轴向（深蹲 / 硬拉）",
 ...
 *
 * 判定完全基于结构化标签：
 * - 轴向下肢负重：movementPattern ∈ {squat_pattern, hinge_pattern} 且 equipment = BARBELL
 * - 过顶按压：movementPattern = vertical_push 且（angle = overhead 或自由重量 BARBELL/DUMBBELL 过顶）
 * - 奥举/爆发：movementPattern = compound 且 equipment = BARBELL（抓举等爆发性杠铃复合动作）
 */
